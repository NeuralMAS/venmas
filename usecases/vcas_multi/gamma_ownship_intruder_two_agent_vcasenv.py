...
class TwoAgentOwnshipIntruderVcasEnv(AbstractMultiAgentEnvironment):
    ...
    def __init__(self, state_space_dim):
        ...
        """
        super(TwoAgentOwnshipIntruderVcasEnv, self).__init__(VcasConstants.BF)

        # Constants.
        self.STATE_SPACE_DIM = state_space_dim
        self.NUM_AGENTS_GAMMA = 2
        self.NUM_AGENTS_NOT_GAMMA = 0

    def get_constraints_for_transition(self, constrs_manager, joint_see_vars_agt,
                                       joint_see_vars_not_agt, input_state_vars):
        """
        Get constraints for a global transition.
        :param constrs_manager:
        :param joint_see_vars_agt:
        :param joint_see_vars_not_agt:
        ...
        # Get each agent's action, assuming Agt = {own, int}, and not(Agt) = {}.
        own_see = joint_see_vars_agt[0]
        int_see = joint_see_vars_agt[1]
        # int_see = joint_see_vars_agt[1] if not joint_see_vars_not_agt[0] else joint_see_vars_not_agt[0]

        own_see_bounds = constrs_manager.get_variable_bounds(own_see)
        int_see_bounds = constrs_manager.get_variable_bounds(int_see)
        ...
        constrs_manager.add_variable_bounds(grb_vars_next_state, HyperRectangleBounds(next_state_lower, next_state_upper))
        ...
        constrs_to_add.extend(
            self.compute_constraints_for_next_state(
                own_advisory, int_advisory, grb_vars_next_state, input_state_vars, constrs_manager))
    ...
    def get_constraints_for_joint_protocol(self, constrs_manager, input_state_vars,
        ...
        # Generate list of indices into joint protocol.
        joint_protocol_indices = list(itertools.product(
            range(self.branching_factor), repeat=len(gamma_see_vars)))

        # Here assuming Gamma = { own, int } and not(Gamma) = { }.
        own_advisory_var = gamma_see_vars[0][0]
        int_advisory_var = gamma_see_vars[1][0]
        ...
        if own_advisory_var_bounds.get_lower() == own_advisory_var_bounds.get_upper() and not isinstance(constrs_manager, CustomConstraintsManager):  # We know the exact advisory
            self.optimise_adv_own(binvars, constrs_manager, constrs_to_add, get_compliance_constraint, grb_vars_next_state, input_state_vars, joint_action_gamma_idx, joint_protocol_indices, next_accelerations_own, own_acc_idx, own_adv_idx, own_advisory_var, own_advisory_var_bounds, own_climbrate_idx)
        else:
        ...
        if int_advisory_var_bounds.get_lower() == int_advisory_var_bounds.get_upper() and not isinstance(constrs_manager, CustomConstraintsManager):  # We know the exact advisory
            self.optimise_adv_intruder(binvars, constrs_manager, constrs_to_add, get_compliance_constraint, grb_vars_next_state, input_state_vars, int_acc_idx, int_adv_idx, int_advisory_var, int_advisory_var_bounds, int_climbrate_idx, joint_action_gamma_idx, joint_protocol_indices, next_accelerations_int)
        else:
            ...
            constrs_to_add.append(
                constrs_manager
                    ...
                    constrs_manager.get_assignment_constraint(
                        ...
                        next_accelerations_int[joint_protocol_indices[joint_action_gamma_idx][1]])))
    ...
    def encode_det_nondet(self, adv, binvars, constrs_manager, constrs_to_add, det, get_compliance_constraint,
                                  grb_vars_next_state, input_state_vars, joint_action_gamma_idx, joint_protocol_indices,
            ...
            constrs_manager.create_indicator_constraint(
                ...
                constrs_manager
                    .get_assignment_constraint(grb_vars_next_state[acc_idx],
                                               next_accelerations[
                                                   joint_protocol_indices[joint_action_gamma_idx][protocol_indices_inner_idx]])))
