...
class TwoAgentIntruderVcasEnv(AbstractMultiAgentEnvironment):
    ...
    def __init__(self, state_space_dim):
        ...
        """
        super(TwoAgentIntruderVcasEnv, self).__init__(VcasConstants.BF)

        # Constants.
        self.STATE_SPACE_DIM = state_space_dim
        self.NUM_AGENTS_GAMMA = 1  # Gamma = { intruder }
        self.NUM_AGENTS_NOT_GAMMA = 1  # not(Gamma) = { ownship }
    ...
    def get_constraints_for_transition(self, constraints_manager, joint_action_vars_gamma, joint_action_vars_not_gamma, state_vars):
        ...
        # Get each agent's action, assuming Agt = {int}, and not(Agt) = {own}.
        int_see = joint_action_vars_gamma[0]
        own_see = joint_action_vars_not_gamma[0]
        ...
        return grb_vars_next_state, constrs_to_add, binvars
    ...
    def get_constraints_for_joint_protocol(self, constrs_manager, input_state_vars,
        ...
        # Here assuming Gamma = { int } and not(Gamma) = { own }.
        ...
        # if own_advisory_var_bounds.get_lower() == own_advisory_var_bounds.get_upper() and not isinstance(constrs_manager, CustomConstraintsManagerWithBounds):  # We know the exact advisory
        #     self.optimise_adv_own(binvars, constrs_manager, constrs_to_add, get_compliance_constraint, grb_vars_next_state, input_state_vars, joint_action_not_gamma_idx, joint_protocol_not_gamma_indices, next_accelerations_own, own_acc_idx, own_adv_idx, own_advisory_var, own_advisory_var_bounds, own_climbrate_idx)
        # else:
        ...
        constrs_to_add.append(
            constrs_manager.create_indicator_constraint(binvars,
                                                        delta_own_adv[VcasConstants.COC],
                                                        1,
        ...
        constrs_to_add.append(
            constrs_manager
                .create_indicator_constraint(binvars,
                                             delta_own_adv[VcasConstants.COC],
                                             1,
                                             constrs_manager.get_assignment_constraint(
                                                            grb_vars_next_state[VcasConstants.OWN_ACC_IDX],
                                                            next_accelerations_own[
                                                                joint_protocol_not_gamma_indices[joint_action_not_gamma_idx][0]])))
        ...
        for adv in range(1, VcasConstants.N_ADVISORIES):
            ...
            constrs_to_add.append(
                constrs_manager.
                    create_indicator_constraint(binvars, delta_own_adv[adv], 1,
            ...
            constrs_to_add.append(
                constrs_manager.get_linear_constraint(
                    [nondet, det, delta_own_adv[adv]], [1, 1, -1], 0, sense=__ge__))
            constrs_to_add.append(
                constrs_manager.get_linear_constraint(
                    [delta_own_adv[adv], det], [1, -1], 0, sense=__ge__))
            constrs_to_add.append(
                constrs_manager.get_linear_constraint(
                    [delta_own_adv[adv], nondet], [1, -1], 0, sense=__ge__))
            ...
            self.encode_det_nondet(adv, binvars, constrs_manager, constrs_to_add, det,
                                   ...
                                   joint_action_not_gamma_idx, joint_protocol_not_gamma_indices, nondet, own_acc_idx,
        ...
        # if int_advisory_var_bounds.get_lower() == int_advisory_var_bounds.get_upper() and not isinstance(constrs_manager, CustomConstraintsManagerWithBounds):  # We know the exact advisory
        #     self.optimise_adv_intruder(binvars, constrs_manager, constrs_to_add, get_compliance_constraint, grb_vars_next_state, input_state_vars, int_acc_idx, int_adv_idx, int_advisory_var, int_advisory_var_bounds, int_climbrate_idx, joint_action_gamma_idx, joint_protocol_gamma_indices, next_accelerations_int)
        # else:
        ...
        constrs_to_add.append(
            constrs_manager.
                create_indicator_constraint(
                    binvars, delta_int_adv[VcasConstants.COC], 1,
        ...
        constrs_to_add.append(
            constrs_manager
                ...
                constrs_manager.get_assignment_constraint(
                    ...
                    next_accelerations_int[joint_protocol_gamma_indices[joint_action_gamma_idx][0]])))
        ...
        for adv in range(1, VcasConstants.N_ADVISORIES):
            ...
            constrs_to_add.append(
                constrs_manager
                    .create_indicator_constraint(binvars, delta_int_adv[adv], 1,
            ...
            constrs_to_add.append(
                constrs_manager.get_linear_constraint(
                    [nondet, det, delta_int_adv[adv]], [1, 1, -1], 0, sense=__ge__))
            constrs_to_add.append(
                constrs_manager.get_linear_constraint(
                    [delta_int_adv[adv], det], [1, -1], 0, sense=__ge__))
            constrs_to_add.append(
                constrs_manager.get_linear_constraint(
                    [delta_int_adv[adv], nondet], [1, -1], 0, sense=__ge__))

            self.encode_det_nondet(adv, binvars, constrs_manager, constrs_to_add, det,
                                   get_compliance_constraint, grb_vars_next_state, input_state_vars,
                                   joint_action_gamma_idx, joint_protocol_gamma_indices, nondet, int_acc_idx,
                                   int_adv_idx, int_advisory_var, int_climbrate_idx, 0)
        return constrs_to_add, binvars

    def optimise_adv_own(self, binvars, constrs_manager, constrs_to_add, get_compliance_constraint, grb_vars_next_state, input_state_vars, joint_action_gamma_idx, joint_protocol_indices, next_accelerations_own, own_acc_idx, own_adv_idx, own_advisory_var, own_advisory_var_bounds, own_climbrate_idx):
        ...
        if adv == VcasConstants.COC:
            ...
            # (1 - det >= 0)
            # (1 - nondet >= 0)
            constrs_manager.get_linear_constraint([nondet, det], [1, 1], 1, sense=__ge__),
            constrs_manager.get_linear_constraint([det], [1], 1, sense=__le__),
            constrs_manager.get_linear_constraint([nondet], [1], 1, sense=__le__)
        # Binary variable for checking whether an acceleration should be chosen
        # non-deterministically.
        [nondet] = constrs_manager.create_binary_variables(1)
        # Binary variable for checking whether an acceleration should not be chosen.
    ...
    def encode_det_nondet(self, adv, binvars, constrs_manager, constrs_to_add, det, get_compliance_constraint,
        ...
        constrs_to_add.append(
            constrs_manager.create_indicator_constraint(
                binvars, det, 1, constrs_manager.get_equality_constraint(
                    advisory_var, input_state_vars[adv_idx])))
        constrs_to_add.append(
            constrs_manager.create_indicator_constraint(
                binvars, det, 1, get_compliance_constraint(adv, input_state_vars[climbrate_idx])))
    ...
    def compute_next_state_upper_bounds(input_variable_bounds_lower, input_variable_bounds_upper, own_see_bounds, int_see_bounds):
        min_own_acc = get_own_min_acc(own_see_bounds.get_lower()[0], own_see_bounds.get_upper()[0])
        max_own_acc = get_own_max_acc(own_see_bounds.get_lower()[0], own_see_bounds.get_upper()[0])
        max_int_acc = get_int_max_acc(int_see_bounds.get_lower()[0], int_see_bounds.get_upper()[0])

        next_state_upper \
            = np.array([min(VcasConstants.MAX_INT_ALT,
                            input_variable_bounds_upper[VcasConstants.INT_ALT_IDX] -
                            input_variable_bounds_lower[VcasConstants.OWN_CLIMBRATE_IDX] - 0.5 * min_own_acc +
                            input_variable_bounds_upper[VcasConstants.INT_CLIMBRATE_IDX] + 0.5 * max_int_acc),
                        min(VcasConstants.MAX_OWN_CLIMBRATE,
                            input_variable_bounds_upper[VcasConstants.OWN_CLIMBRATE_IDX] + max_own_acc),
                        min(VcasConstants.MAX_INT_CLIMBRATE,
                            input_variable_bounds_upper[VcasConstants.INT_CLIMBRATE_IDX] + max_int_acc),
    ...
    def compute_next_state_lower_bounds(input_variable_bounds_lower, input_variable_bounds_upper, own_see_bounds, int_see_bounds):
        min_own_acc = get_own_min_acc(own_see_bounds.get_lower()[0], own_see_bounds.get_upper()[0])
        max_own_acc = get_own_max_acc(own_see_bounds.get_lower()[0], own_see_bounds.get_upper()[0])
        min_int_acc = get_int_min_acc(int_see_bounds.get_lower()[0], int_see_bounds.get_upper()[0])

        next_state_lower \
          = np.array([max(VcasConstants.MIN_INT_ALT, input_variable_bounds_lower[VcasConstants.INT_ALT_IDX] -
                          input_variable_bounds_upper[VcasConstants.OWN_CLIMBRATE_IDX] - 0.5 * max_own_acc +
                          input_variable_bounds_lower[VcasConstants.INT_CLIMBRATE_IDX] + 0.5 * min_int_acc),
                      max(VcasConstants.MIN_OWN_CLIMBRATE,
                          input_variable_bounds_lower[VcasConstants.OWN_CLIMBRATE_IDX] + min_own_acc),
                      max(VcasConstants.MIN_INT_CLIMBRATE,
                          input_variable_bounds_lower[VcasConstants.INT_CLIMBRATE_IDX] + min_int_acc),
...
def get_own_min_acc(advisory_lower, advisory_upper):
    lower = VcasConstants.MAX_OWN_ACC
    for adv in range(int(advisory_lower), int(advisory_upper)+1):
        lower = min(VcasConstants.ADV_INFO[adv].accelerations[0], lower)

    return  lower


def get_int_min_acc(advisory_lower, advisory_upper):
    lower = VcasConstants.MAX_INT_ACC
    for adv in range(int(advisory_lower), int(advisory_upper) + 1):
        lower = min(VcasConstants.ADV_INFO[adv].accelerations[0], lower)
...
def get_own_max_acc(advisory_lower, advisory_upper):
    upper = VcasConstants.MIN_OWN_ACC
    for adv in range(int(advisory_lower), int(advisory_upper)+1):
        upper = max(VcasConstants.ADV_INFO[adv].accelerations[-1], upper)

    return  upper


def get_int_max_acc(advisory_lower, advisory_upper):
    lower = VcasConstants.MIN_INT_ACC
    for adv in range(int(advisory_lower), int(advisory_upper) + 1):
        lower = max(VcasConstants.ADV_INFO[adv].accelerations[-1], lower)
